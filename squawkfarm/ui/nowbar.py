...
class NowBar(InstructionGroup):
    def __init__(self, start_x, end_x, bottom_y, top_y, duration, color=Color(0.05, 0.05, 0.3, 1), width=3.0, loop=False):
        ...
        self.color = color
        self.line = Line(points=[self.start_x, self.bottom_y, self.start_x, self.top_y], width=width)
    ...
    def _update_line(self):
        x = self.start_x + (self.end_x - self.start_x) * (self.current_time / self.duration)

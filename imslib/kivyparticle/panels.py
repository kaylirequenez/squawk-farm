...
class ViewPanel(StencilView):
    def __init__(self):
        ...
        self.particle.emitter_y = (
            -200
        )  # particle will be centered once layout is loaded
        self.particle.start()
    ...
    def update_param(self, param_name, param_value, param_label=None):
        ...
        if param_name == "texture":
            ...
            # TODO fix
            self.particle.texture_path = os.path.join(
                os.getcwd(), "particle", param_value
            )
    ...
    def save_config(self, config_name, save_path):
        ...
        if not os.path.exists(os.path.join(os.getcwd(), self.texture + ".png")):
            shutil.copyfile(
                self.particle.texture_path,
                os.path.join(os.getcwd(), self.texture + ".png"),
            )
...
class SavePopup(Popup):
    def __init__(self, save_callback, pex_error_callback):
        ...
        self.filechooser = FileChooserIconView(path=os.getcwd(), size_hint=(1, 0.8))
        self.config_name = TextInput(
            hint_text="config.pex", multiline=False, size_hint=(0.3, 0.1)
        )
        self.save_bt = Button(
            text="Save",
            size_hint=(0.1, 0.1),
...
class LoadPopup(Popup):
    def __init__(
        self, load_callback, pex_error_callback, error_callback, on_new_config
    ):
        ...
        self.filechooser = FileChooserIconView(path=os.getcwd(), size_hint=(0.8, 0.8))
        self.load_bt = Button(
            text="Select",
            size_hint=(0.1, 0.1),
            background_normal="button6.png",
...
class GeneralPanel(BoxLayout):
    def __init__(self, particle, on_new_config, on_randomize):
        ...
        self.pex_error_message = Label(
            text="Particle system only accepts .pex files", size_hint=(0.1, 0.1)
        )
        self.pex_error_popup = Popup(
            title="Error",
            content=self.pex_error_message,
            size_hint=(0.5, 0.2),
            pos_hint={"center_x": 0.5, "center_y": 0.5},
        )
        self.fail_error_message = Label(
            text="Config file cannot be loaded", size_hint=(0.1, 0.1)
        )
        self.fail_error_popup = Popup(
            title="Error",
            content=self.fail_error_message,
            size_hint=(0.5, 0.2),
            pos_hint={"center_x": 0.5, "center_y": 0.5},
        )
        ...
        self.save_popup = SavePopup(
            save_callback=self.ps.save_config,
            pex_error_callback=self.pex_error_popup.open,
        )

        # popup for loading file
        self.load_popup = LoadPopup(
            load_callback=self.ps.load_config,
            pex_error_callback=self.pex_error_popup.open,
            error_callback=self.fail_error_popup.open,
        ...
        self.edit = Button(
            text="Texture: {}".format(self.ps.texture),
            size_hint_y=None,
            height=150,
            pos_hint={"top": 1},
            background_normal="button6.png",
...
class ParticleConfigPanel(BoxLayout):
    def __init__(self, particle):
        ...
        self.add_widget(
            Label(text="Particle Configuration", size_hint_y=None, height=100)
        )

        for param in PARTICLE_PARAMETERS:
            slider = ParamSlider(
                param,
                default=get_param_default(self.ps.particle, param["name"]),
...
class EmitterConfigPanel(BoxLayout):
    def __init__(self, particle):
        ...
        if self.ps.particle.emitter_type == 0:
            ...
            self.bt = Button(
                text="Emitter Type: {}".format(self.emitter_type),
                size_hint_y=None,
                height=100,
                background_normal="button6.png",
                background_down="button6down.png",
        ...
        for slider in self.radial_sliders:
            ...
            if slider.name == "min_radius":
                value = random.uniform(
                    slider.min, min(slider.max, self.ps.particle.max_radius)
                )
                slider.set_value(value)
            else:
                value = random.uniform(slider.min, slider.max)
        ...
        for param in START_COLOR_PARAMETERS:
            slider = ParamSlider(
                ...
                default=get_param_default(
                    self.ps.particle, param["name"], param["label"]
                ),
                callback=self.ps.update_param,
            )
            self.sliders.append(slider)
            ...
            if START_COLOR_PARAMETERS.index(param) == 3:
                self.add_widget(
                    Label(text="Start Color Variance", size_hint_y=None, height=50)
                )
    ...
    def __init__(self, particle):
        ...
        for param in END_COLOR_PARAMETERS:
            ...
            if END_COLOR_PARAMETERS.index(param) == 3:
                self.add_widget(
                    Label(text="End Color Variance", size_hint_y=None, height=50)
                )

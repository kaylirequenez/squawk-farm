...
from core import BaseWidget, lookup, run
...
class ParamSlider(GridLayout):
    ...
    def __init__(self, parameter, default, callback=None):
        ...
        self.param_label = Label(
            ...
            font_size=20,
            size_hint_x=0.3,
            on_texture=self.change_font_size,
        )
        ...
        self.slider = Slider(
            min=self.min,
            max=self.max,
            value=self.clip_value(default),
            step=self.step,
            size_hint_x=0.6,
            cursor_size=("20sp", "20sp"),
            cursor_image="slider2.png",
        )
        self.add_widget(self.slider)

        # param value
        self.param_value = Label(
            text=str(self.slider.value), font_size=20, size_hint_x=0.1
        )
